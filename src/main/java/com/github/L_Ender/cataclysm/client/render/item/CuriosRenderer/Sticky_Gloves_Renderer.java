...
public class Sticky_Gloves_Renderer implements ICurioRenderer {
    ...
    public static Sticky_Gloves_Renderer getGloveRenderer(ItemStack stack) {
        if (!stack.isEmpty()) {
            return CuriosRendererRegistry.getRenderer(stack.getItem())
                    .filter(Sticky_Gloves_Renderer.class::isInstance)
                    .map(Sticky_Gloves_Renderer.class::cast)
                    .orElse(null);
        }
        return null;
    }
    ...
    protected void renderArm(Sticky_Gloves_Model model, PoseStack matrixStack, MultiBufferSource buffer, HumanoidArm handSide, int light, boolean hasFoil) {
        ...
        model.renderArm(handSide, matrixStack, vertexBuilder, light, OverlayTexture.NO_OVERLAY,1.0f,1.0f,1.0f,1.0f);
    }
    ...
    protected void renderFirstPersonArm(Sticky_Gloves_Model model, ModelPart arm, PoseStack matrixStack, MultiBufferSource buffer, int light, boolean hasFoil) {
        ...
        arm.render(matrixStack, builder, light, OverlayTexture.NO_OVERLAY,1.0F,1.0F,1.0F,1.0F);
    }
