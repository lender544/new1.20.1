...
import com.github.L_Ender.cataclysm.client.particle.Options.Circle_ParticleOptions;
 ...
 */
public class Circle_Particle extends TextureSheetParticle {
    public int r, g, b;
    ...
    public void render(VertexConsumer buffer, Camera renderInfo, float partialTicks) {
        ...
        float fadeProgress = (smoothAge - opacitystart) / ((float)this.lifetime - opacitystart);

        float fadeMultiplier = 1.0F - fadeProgress;

        fadeMultiplier = Mth.clamp(fadeMultiplier, 0.0f, 1.0f);

        alpha = opacity * (0.95f * fadeMultiplier + 0.05f);
    ...
    @OnlyIn(Dist.CLIENT)
    public static class CircleFactory implements ParticleProvider<Circle_ParticleOptions> {
        ...
        public Particle createParticle(Circle_ParticleOptions typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            Circle_Particle particle = new Circle_Particle(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, typeIn.yaw(), typeIn.pitch(), typeIn.duration(), typeIn.r(), typeIn.g(), typeIn.b(), typeIn.a(), typeIn.aStart(),typeIn.scale(), typeIn.facesCamera(), typeIn.behavior(),spriteSet);
            return particle;
        }
    }
