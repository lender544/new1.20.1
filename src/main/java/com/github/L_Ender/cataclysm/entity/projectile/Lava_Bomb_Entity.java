...
public class Lava_Bomb_Entity extends ThrowableProjectile {
    ...
    protected void doTerrainEffects() {
        ...
        while (landed.getY() < level().getMaxBuildHeight() && (!level().getBlockState(landed).isAir() || !level().getBlockState(landed).getFluidState().isEmpty() && level().getBlockState(landed).getFluidState().getFluidType() != net.minecraftforge.common.ForgeMod.LAVA_TYPE.get())) {
            landed = landed.above();
        }
        setLavaPos(landed);
    ...
    public void remove(RemovalReason reason) {
        ...
        if (!this.level().isClientSide) {
            if (this.getLavaPos() != BlockPos.ZERO) {
                if (level().getFluidState(this.getLavaPos()).getFluidType() == net.minecraftforge.common.ForgeMod.LAVA_TYPE.get()) {
