...
public class SemiAquaticAIRandomSwimming extends RandomStrollGoal {
    ...
    protected Vec3 getPosition() {
        ...
        for(int i = 0; vector3d != null && !this.mob.level().getFluidState(BlockPos.containing(vector3d)).is(FluidTags.LAVA) && !this.mob.level().getBlockState(BlockPos.containing(vector3d)).isPathfindable(this.mob.level(), BlockPos.containing(vector3d), PathComputationType.WATER) && i++ < 15; vector3d = DefaultRandomPos.getPos(this.mob, 10, 7)) {
        }
