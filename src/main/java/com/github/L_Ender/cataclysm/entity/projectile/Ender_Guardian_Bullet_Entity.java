...
public class Ender_Guardian_Bullet_Entity extends AbstractHurtingProjectile {
    ...
    public Ender_Guardian_Bullet_Entity(Level worldIn, LivingEntity shooter, Vec3 accel) {
        super(ModEntities.ENDER_GUARDIAN_BULLET.get(), shooter, accel, worldIn);
    }
    ...
    public void tick() {
        ...
        if (level().isClientSide || (shooter == null || !shooter.isRemoved()) && level().hasChunkAt(this.blockPosition())) {
            ...
            if (HitResult.getType() != net.minecraft.world.phys.HitResult.Type.MISS && !net.neoforged.neoforge.event.EventHooks.onProjectileImpact(this, HitResult)) {
                onHit(HitResult);
            }
