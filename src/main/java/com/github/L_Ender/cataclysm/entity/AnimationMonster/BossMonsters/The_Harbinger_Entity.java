...
public class The_Harbinger_Entity extends LLibrary_Boss_Monster implements RangedAttackMob, PowerableMob {
            ...
            if (!this.level().isClientSide) {
                if (net.neoforged.neoforge.event.EventHooks.canEntityGrief(this.level(), this)) {
                    ...
                    for (BlockPos blockpos : BlockPos.betweenClosed(Mth.floor(aabb.minX), Mth.floor(aabb.minY), Mth.floor(aabb.minZ), Mth.floor(aabb.maxX), Mth.floor(aabb.maxY), Mth.floor(aabb.maxZ))) {
                        ...
                        if (!blockstate.isAir() && blockstate.canEntityDestroy(this.level(), blockpos, this) && !blockstate.is(ModTag.HARBINGER_IMMUNE) && net.neoforged.neoforge.event.EventHooks.onEntityDestroyBlock(this, blockpos, blockstate)) {
    ...
    private void destoryblock2() {
        ...
        if (this.getAnimation() == NO_ANIMATION) {
            if (!getIsCharge()) {
                if (!level().isClientSide && this.blockBreakCounter == 0 && net.neoforged.neoforge.event.EventHooks.canEntityGrief(level(), this)) {
    ...
    static class MissileLaunchGoal extends SimpleAnimationGoal<The_Harbinger_Entity> {
        ...
        private void mlaunch(int head, LivingEntity target) {
            ...
            Wither_Homing_Missile_Entity laserBeam = new Wither_Homing_Missile_Entity(entity, vec3,entity.level(),(float) CMConfig.WitherHomingMissiledamage,target);
