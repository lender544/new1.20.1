...
public class CMRecipes
{
	...
	public List<WeaponfusionRecipe> getWeaponfusionRecipes() {
		return recipeManager.getAllRecipesFor(ModRecipeTypes.WEAPON_FUSION.get()).stream().map(RecipeHolder::value).toList();
	}

	public List<AltarOfAmethystRecipe> getAmethystBlessRecipes() {
		return recipeManager.getAllRecipesFor(ModRecipeTypes.AMETHYST_BLESS.get()).stream().map(RecipeHolder::value).toList();
	}
