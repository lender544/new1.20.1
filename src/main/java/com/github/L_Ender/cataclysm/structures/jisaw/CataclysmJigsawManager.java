...
public class CataclysmJigsawManager {
    ...
    public static Optional<Structure.GenerationStub> addPieces(
        ...
        if (structurepoolelement == EmptyPoolElement.INSTANCE) {
            ...
            if (structurepoolelement instanceof CataclysmPoolElement) {
                offset = ((CataclysmPoolElement) structurepoolelement).bounds.offset;
            ...
            if (!predicate.test(chunkgenerator.getBiomeSource().getNoiseBiome(QuartPos.fromBlock(centerX), QuartPos.fromBlock(height), QuartPos.fromBlock(centerZ), context.randomState().sampler()))) {
                ...
                return Optional.of(new Structure.GenerationStub(new BlockPos(centerX, height, centerZ), (builder) -> {
                    ...
                    if (maxDepth >= 0) {
                        ...
                        String tag = null;
                        if (poolelementstructurepiece.getElement() instanceof CataclysmPoolElement) tag = ((CataclysmPoolElement) poolelementstructurepiece.getElement()).getRandomTag(worldgenrandom);
                        ...
                        list.sort((p1, p2) -> {
                            int i1, i2;
                            i1 = i2 = 0;
                            if (p1.getElement() instanceof CataclysmPoolElement) i1 = ((CataclysmPoolElement) p1.getElement()).genOrder;
                            if (p2.getElement() instanceof CataclysmPoolElement) i2 = ((CataclysmPoolElement) p2.getElement()).genOrder;
    ...
    static class Placer {
        ...
        PieceSelection selectPiece(PieceState pieceState, StructureTemplatePool poolOptional, StructureTemplatePool fallbackPoolOptional, boolean villageBoundaryAdjust, StructureBlockInfo thisPieceJigsawBlock, LevelHeightAccessor heightAccessor, RandomState state) {
            ...
            structurePoolElements.sort((p1, p2) -> {
                int i1, i2;
                i1 = i2 = 0;
                if (p1 instanceof CataclysmPoolElement) i1 = ((CataclysmPoolElement) p1).priority;
                if (p2 instanceof CataclysmPoolElement) i2 = ((CataclysmPoolElement) p2).priority;
                return Integer.compare(i1, i2);
            });
...
//                if (nextPieceCandidate.toString().contains("room") || nextPieceCandidate.toString().contains("tower")) continue;
                ...
                if (nextPieceCandidate instanceof CataclysmPoolElement) {
                    CataclysmPoolElement CataclysmPoolElement = (CataclysmPoolElement) nextPieceCandidate;
                    if (!CataclysmPoolElement.checkCriteria(pieceState, this)) continue;
                }
                ...
                for (Rotation nextPieceRotation : Rotation.getShuffled(this.random)) {
                    ...
                    for (StructureBlockInfo nextPieceJigsawBlock : nextPieceJigsawBlocks) {
                        boolean canAttach;
                        if (nextPieceCandidate instanceof CataclysmPoolElement && ((CataclysmPoolElement) nextPieceCandidate).twoWay()) {
                            canAttach = CataclysmPoolElement.canAttachTwoWays(thisPieceJigsawBlock, nextPieceJigsawBlock);
                        } else {
                        ...
                        if (canAttach) {
                            ...
                            if (nextPieceCandidate instanceof CataclysmPoolElement) {
                                nextPieceMinY -= ((CataclysmPoolElement) nextPieceCandidate).bounds.boundsMinOffset.getY();
                            }
                            ...
                            // Check height params
                            if (nextPieceCandidate instanceof CataclysmPoolElement) {
                                Optional<Integer> maxHeight = ((CataclysmPoolElement) nextPieceCandidate).conditions.maxHeight;
                                Optional<Integer> minHeight = ((CataclysmPoolElement) nextPieceCandidate).conditions.minHeight;
                            ...
                            if (checkBounds(nextPieceCandidate, pieceState, nextPieceBoundingBoxPlaced, nextPiecePos, nextPieceRotation, i2)) {
                                ...
                                if (nextPieceCandidate instanceof CataclysmPoolElement) {
                                    ...
                                    Optional<String> needsOverlapBounds = CataclysmPoolElement.bounds.needsOverlapBounds;
                                    ...
                                    Optional<String> forbiddenOverlapBounds = CataclysmPoolElement.bounds.forbiddenOverlapBounds;
                                ...
                                if (nextPieceCandidate instanceof CataclysmPoolElement) {
                                    interiorBounds = ((CataclysmPoolElement) nextPieceCandidate).getInteriorBoundingBox(this.structureManager, nextPiecePos, nextPieceRotation);
        ...
        protected boolean checkBounds(StructurePoolElement nextPieceCandidate, PieceState pieceState, BoundingBox nextPieceBoundingBoxPlaced, BlockPos nextPiecePos, Rotation nextPieceRotation, int i2) {
            ...
            if (nextPieceCandidate instanceof CataclysmPoolElement) {
                ignoreBounds = ((CataclysmPoolElement) nextPieceCandidate).ignoresBounds();
                spaceCheckBounds = ((CataclysmPoolElement) nextPieceCandidate).getCheckBoundingBox(this.structureManager, nextPiecePos, nextPieceRotation).moved(0, i2, 0);
            }
        ...
        void addNextPieceState(PieceSelection pieceSelection) {
            ...
            if (!(pieceSelection.nextPiece instanceof CataclysmPoolElement && (((CataclysmPoolElement) pieceSelection.nextPiece).ignoresBounds() || !((CataclysmPoolElement) pieceSelection.nextPiece).placeBounds()))) {
            ...
            if (pieceSelection.nextPiece instanceof CataclysmPoolElement && pieceSelection.nextPieceInteriorBoundingBox != null) {
            ...
            if (pieceSelection.nextPiece instanceof CataclysmPoolElement) {
                CataclysmPoolElement CataclysmPoolElement = (CataclysmPoolElement) pieceSelection.nextPiece;
                Optional<String> nextPieceSpecialBounds = CataclysmPoolElement.bounds.specialBounds;
            ...
            if (pieceSelection.nextPiece instanceof CataclysmPoolElement) {
                CataclysmPoolElement CataclysmPoolElement = (CataclysmPoolElement) pieceSelection.nextPiece;
                if (CataclysmPoolElement.tags.inheritsTag) tag = pieceSelection.pieceState.tag;
                else tag = ((CataclysmPoolElement) pieceSelection.nextPiece).getRandomTag(random);
            }
            ...
            if (!(pieceSelection.poolelementstructurepiece.getElement() instanceof CataclysmPoolElement) || !((CataclysmPoolElement) pieceSelection.poolelementstructurepiece.getElement()).twoWay()) {
            ...
            if (pieceSelection.nextPiece instanceof CataclysmPoolElement && ((CataclysmPoolElement) pieceSelection.nextPiece).conditions.numPathsOverride.isPresent()) {
                numPaths += ((CataclysmPoolElement) pieceSelection.nextPiece).conditions.numPathsOverride.get();
            }
            else {
                ...
                for (Rotation nextPieceRotation : Rotation.getShuffled(this.random)) {
                    ...
                    for (StructureBlockInfo nextPieceJigsawBlock : nextPieceJigsawBlocks) {
                        ...
                        if (canAttach) {
                            ...
                            // Check height params
                            if (nextPieceCandidate instanceof CataclysmPoolElement) {
                                Optional<Integer> maxHeight = ((CataclysmPoolElement) nextPieceCandidate).maxHeight;
                                Optional<Integer> minHeight = ((CataclysmPoolElement) nextPieceCandidate).minHeight;
                                if (maxHeight.isPresent() || minHeight.isPresent()) {
